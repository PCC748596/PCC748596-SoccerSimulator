...
const PosicionamentoAI = {
    ...
    */
    tickBase: function (p, bb) {
        ...
        */
        ...
        // No tiro de meta a posicao e puramente a do TeamBT (slot no bloco)
        if (typeof Match !== 'undefined' && Match.state === 'GOAL_KICK') {
    ...
    */
    tickFinal: function (p, bb) {
        ...
        if (typeof Match !== 'undefined' && Match.state === 'GOAL_KICK') {
            const tx = THREE.MathUtils.clamp(p.postoBase.x, -34, 34);
            const tz = THREE.MathUtils.clamp(p.postoBase.z, -50, 50);
            if (!p.tacticalTarget) p.tacticalTarget = new THREE.Vector3(tx, ALTURA_BASE_Y, tz);
            p.tacticalTarget.set(tx, ALTURA_BASE_Y, tz);
            if (!p.styleTarget) p.styleTarget = new THREE.Vector3(tx, ALTURA_BASE_Y, tz);
            p.styleTarget.set(tx, ALTURA_BASE_Y, tz);
            p.dynamicTarget.set(tx, ALTURA_BASE_Y, tz);
            if (typeof proporAlvo === 'function') {
                proporAlvo(p, AlvoPrio.LEIS, tx, tz, 'teamBT:goalKick');
            }
            return;
        }
