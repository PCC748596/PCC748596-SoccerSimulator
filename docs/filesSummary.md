...
- **Intenções de jogada implementadas (`js/intentions.js`, novo + `IntentModel`, config.js):** o recetor anuncia a rota (`Ball_Request` — direcção, velocidade, duração) e cumpre-a; o passador projecta o ponto de encontro sobre essa promessa (`Long_Ball_Inform`) em vez de adivinhar; qualquer um pode quebrar (`Intent_Cancel`, com motivo). Registo por equipa com relógio próprio, três quebras revalidadas por frame (posse perdida, fora-de-jogo, rota fechada), telemetria em `MatchStats.contratos`, e **braço levantado** enquanto o contrato está aberto (`aplicarCamadaPedirBola`, js/player.js). Novo ramo `PedirBola` e `CumprirContrato` na árvore sem bola, `ServirContrato` na árvore com bola. Testes: `tests/intencoes.test.js`. **Estado honesto: o sistema corre e não melhora nada ainda** — A/B mede 57.9% de acerto com as intenções ligadas contra 59.6% desligadas, e em 96 s de jogo nascem ~120 contratos mas só 7 são servidos e quase nenhum cumprido. O gargalo está na taxa de serviço, não no mecanismo; ver o que se aprendeu, abaixo.
- **O que se aprendeu a ligar as intenções, e que é preciso saber antes de lhes mexer:** (1) o `findThroughBall` **é praticamente código morto** — o único caminho que lhe chega, `findBestPassAnywhere`, corre 11 vezes em 48 s e **sempre sob pressão**, e o ramo do lançamento nunca é alcançado; por isso os contratos foram pendurados num ramo próprio (`ServirContrato`) e não ali. (2) Os pontos de encontro caem a **7-14 m** da bola, não aos 18+ que o spec assumia — quem pede a bola está perto do portador; com `distMin` em 14, 14 de 15 projecções eram recusadas. (3) A regra de "rota fechada" mata tudo se for ingénua: como o destino está à frente, há sempre um defesa mais perto dele do que o próprio dono — precisa de raio (`raioRota`), margem de tempo (`margemRota`) e graça no arranque (`graca`), e só se aplica a contratos ainda **abertos**: depois de servido, cancelar só faz o jogador desistir de uma bola que já vai a caminho.
- **O lead do passe: extrapolar é PIOR do que não extrapolar (`alvoDePasse`, js/utils.js + `LeadModel`, config.js):** varrimento do factor de curvatura (8 sementes × 180 s, taxa de passe completo): **0.00 → 63.2%**, 0.35 → 62.7%, 0.55 → 61.8%, 0.82 → 59.3%. Quanto mais lead, pior: o ponto à frente está mais perto dos adversários, o passe fica mais longo, e o colega muda de ideias a qualquer frame. É a justificação medida do sistema de intenções — adivinhar o futuro de quem não se comprometeu é pior do que não adivinhar. Ficou em `0.35`, que mantém a noção de passar à frente por um custo dentro do ruído. As três correcções entraram na mesma: velocidade **média** recente do recetor (`p.velMedia`, EMA com `tauVelMedia`) em vez da instantânea, factor de curvatura, e `vBall = 14.0` substituído por **`tempoDeVoo(d)`** (novo, utils.js), que devolve o tempo que a física do passe já determina — 5 m dava 0.36 s com a constante antiga contra 0.81 s reais.
- **O bónus de "sem marcação" passou a saber a direcção (`PassBonus`, config.js):** era cego — um defesa livre atrás valia os mesmos pontos que um avançado livre à frente, e o defesa atrás está quase sempre mais livre. Era o "o atacante dispara sozinho e toca para trás". `PassBonus.lado` (0.75) e `PassBonus.atras` (0.35) escalam o bónus pela progressão do passe. Medido: passes para trás **48.3% → 41.8%**, com a taxa de passe igual dentro do ruído. Recuar continua a ser legítimo; só deixou de valer o mesmo que progredir.
- **Velocidade de condução própria (`CarryModel.velocidade`, config.js):** o `actCarry` não definia velocidade nenhuma — só `changeState('CARRY')` — e a FSM corria com `speedMult * 0.95` do ramo anterior. Medido, o portador era o mais lento do campo tirando quem bloqueava: **6.59 m/s** contra 8.61 de quem corre ao espaço e 6.85 de quem intercepta, com a dispersão a vir de que ramo passara por ele antes. Agora `{ base: 6.45, porSkill: 1.4 }` na forma dos outros ramos, calibrada em ~92% da corrida ao espaço, e o `* 0.95` da FSM saiu (era um corte sobre um valor herdado). Medido depois: **8.32 m/s**. Testes: `tests/velocidade_conducao.test.js`.
- **Tecto de velocidade angular (`SteeringModel.giroMaxGrausPorSeg`, config.js):** a viragem era um slerp exponencial sem limite — a velocidade angular é proporcional ao erro, e numa inversão de 180° dava **3240 °/s** (52° num único frame). O `passoDeGuinada` (utils.js) existia para isto desde sempre, com tecto e **cinco testes a passar, e nunca tinha sido chamado por ninguém** — o mesmo padrão do `bb.phase` e do `findPassTargetInCone`. Ligado ao `steerArrive`, com tecto de **180 °/s** por escolha do dono do projecto (meia volta por segundo; um humano vira-se a 400-600). Custo medido na taxa de passe: nenhum fora do ruído.

- **Não se rouba a bola pelas costas (`podeDisputarBola`/`alvoDePerseguicao`, js/utils.js + `ProtecaoDeBola`, js/config.js):** o `resolveBallContact` escolhia o jogador mais PRÓXIMO da bola e mais nada — e com a bola lenta (`speed < BallControl.easySpeed`) o domínio é automático, sem disputa. Um adversário que chegasse por trás e ficasse 10 cm mais perto levava a bola. Agora, com dono, só disputa quem estiver **lado a lado ou à frente**: mede-se o avanço do desafiante ao longo da direcção em que o dono se move, e mais de `recuoMax` (0.35 m) atrás não chega à bola. Vale entre companheiros também. A contraparte é o `alvoDePerseguicao`: quem vem de trás deixa de mirar o mesmo ponto que o portador e mira **ao lado** (`offsetLateral`, 1.2 m) para emparelhar e ultrapassar; só vai a direito à bola se vier de frente (`frenteMin`). **Armadilha apanhada a medir:** a primeira versão considerava dono quem tivesse `carryTouchGrace > 0`, e o passador mantém essa graça uns décimos depois de a bola sair — a protecção passou a impedir o DESTINATÁRIO de tocar na bola (ele está longe, logo "atrás" no eixo do movimento do passador) e a taxa de passe caiu de 65% para 53.6%. Só há dono com a bola a menos de `raioPosse` (3.6 m, que cobre o toque longo de condução) e sem passe em curso. Medido A/B: protecção **ligada 51.5% de acerto e 635 passes**, desligada 48% e 304 — o jogo flui melhor com ela. Testes: `tests/protecao_de_bola.test.js`.
- **A inércia do movimento vale 13 pontos de taxa de passe (`SteeringModel`, js/config.js):** desligar o amortecimento do `steerArrive` (`inercia: false`, pedido para se ver o jogo sem o atraso da suavização) fez a taxa de passe cair de **65.9% para 53.0%**, com os cortes a subir para ~43%: sem inércia os defesas mudam de direcção no próprio frame e chegam a bolas que antes não alcançavam. Medida a curva e escolhido o meio-termo — `aceleracao` 5.0 → **12.0** e `giro` 10.0 → **18.0**, que dá mais do dobro da responsividade com o mesmo acerto (60.5% contra 61.0%). A partir de 18/24 a curva parte (56.8%). Os dois números estavam escondidos à mão dentro do `player.js` e passaram para o config, com a tabela da medição ao lado.
- **O passe para trás é metade dos passes, e o bónus de "sem marcação" é cego à direcção (`PassBonus`, js/config.js):** queixa de "o atacante dispara sozinho e toca para trás". Medido: **48.3%** dos passes vão para trás — e já iam antes de se mexer em nada. A causa é o bónus de recetor desmarcado, que dá os mesmos pontos a um defesa livre atrás e a um avançado livre à frente; e um defesa atrás está quase sempre mais livre. Subir o bónus (300→400 / 100→200, pedido explícito) não mudou o acerto (54.7% contra 54.4%) mas empurrou os passes para trás para **51.7%**, e foi revertido a pedido. Os valores saíram do meio do `player.js` para `PassBonus` no config, para poderem ser medidos A/B. **Por resolver:** escalar o bónus pela direcção, para um colega livre à frente valer mais do que um livre atrás.
- **Onde o lead do passe erra (`alvoDePasse`, js/utils.js) — medido, por corrigir (`tools/diag_lead.js`, novo):** três erros que se multiplicam. (1) A velocidade do recetor é a **instantânea** e ele quase sempre abranda: assumida 7.11 m/s contra 6.40 reais, e **em 67% dos passes vai mais devagar do que o assumido**. (2) A extrapolação é em **linha recta** e ele curva: deslocamento recto 4.9 m contra 6.4 m de caminho andado (p10 = 0.39 — num em cada dez anda mais do dobro do que se desloca). Composto, o ponto de lead fica **~35% mais adiantado** do que devia, sempre para a frente, e é por isso que a bola passa à frente dele. (3) Em sentido contrário, `vBall = 14.0` constante subestima o tempo de voo (a média real ronda 7.6 m/s), encurtando o lead — dois erros a anularem-se em parte, que é porque afinar isto por tentativa nunca convergiu.

- **"A linha de passe aparece mas o passe sai noutra direcção" — a linha é que estava errada (`Match.atualizarVisuaisDePasse`, js/match.js):** cinco hipóteses foram medidas e caíram todas (`tools/diag_linha_passe.js`, novo): o desvio entre a linha e a saída da bola tem **mediana 2.3° e máximo 28°** (é só o erro de execução), **0%** dos passes são abortados entre a decisão e o contacto, **0%** têm o contacto recusado pela guarda `hasBall/passTarget`, e **0%** se sobrepõem a outro passe. O defeito estava na UI: os botões **Team BT POS** e **PlayingStyleBT** (js/main.js) punham `passLineVisual.visible = true` **sem redesenhar nada** — o que aparecia era a linha do ÚLTIMO passe desenhado, ancorada num jogador e num alvo já velhos, sem relação com passe nenhum a acontecer. A visibilidade passou a viver num sítio só (`Match.passVisualAtivo` + `atualizarVisuaisDePasse`): só há linha se houver passe em curso E o painel estiver ligado; os toggles apenas dizem "quero ver". **Armadilha encontrada pelo caminho:** a condição era `window.teamBTPosState !== 'OFF'`, que dá LIGADO para `undefined` — e os dois estados só são inicializados em `main.js`, que o harness headless e a simulação em lote não carregam.
- **A linha do passe passou a durar o voo e a partir da BOLA (`Match.desenharLinhaDePasse`):** era apagada no instante do contacto, ou seja só existia durante os 0.08 s entre a decisão e o toque — tempo nenhum para se ver o que quer que fosse. Agora fica até a bola ser dominada, parar (< 0.6 m/s) ou o jogo parar. E parte de onde a **bola** está, não do jogador: a bola sai de onde está pousada (medido, 0.55 m à frente do pé), e num passe curto isso dava até 28° entre a linha desenhada e a trajectória real — sem erro de execução nenhum pelo meio.
- **Força dos passes para a ala: medida, e está calibrada.** Queixa de que "os passes do meio para as laterais têm força para sair do campo". Medido onde a bola pararia sozinha, só com o atrito de rolamento, contra a distância pedida: **pedido 12.1 m → pararia a 14.6 m** para alvos na ala (13.9 → 14.4 no meio), ou seja pouco mais de 1 m de excesso, que é o que a calibração por velocidade de chegada prevê. Só **4%** dos passes para a ala passariam mais de 6 m além do alvo.
- **Não existe lançamento lateral, e a `BarreiraCampo` está em cima da linha (js/config.js):** `BarreiraCampo.x = CAMPO_LARG / 2` = 34 m, exactamente a linha lateral — a bola bate numa parede invisível na própria linha e ressalta para dentro (`restituicao: 0.35`). É a "barreira limitante". É deliberado: as únicas bolas paradas implementadas são `CORNER_KICK` e `GOAL_KICK`, não há `THROW_IN`. Medido: a bola encosta-lhe **3.8 frames por minuto** de jogo. Enquanto não houver lançamento lateral, um passe que devia sair pela linha volta ao campo a ressaltar — e é isso que se vê como "a bola foi para outro lado".

- **As posturas do TeamBT foram removidas (`TeamPosture`/`setPosture`, js/bt/team_bt.js):** eram nove (SET_PIECE, BUILD_UP, ATTACK_SUSTAINED, FINAL_THIRD, COUNTER, HIGH_PRESS, MID_BLOCK, LOW_BLOCK, FLANK_SHIFT), escritas em `bb.posture` pela árvore — e **sem um único consumidor no jogo**: o `TeamPostureTuning`, que era quem as lia para deslocar o bloco, já tinha sido apagado, e a partir daí o único sítio do projecto que lia `bb.posture` era o fluxograma da aba separada, que as mostrava como se fossem o resultado da decisão. Quem manda hoje é o `TeamState` (Offensive / T.Offensive / Defensive / T.Defensive), calculado em `gather()` e lido pelos Playing Styles e pela marcação. A árvore ficou só com as condições (que continuam a classificar a situação e a acender no trace) e o `lerAmeacaDeFlanco`, que é a única acção com efeito real. Saiu junto o ramo **`PosseInstalada`**, cuja condição era `posseProlongada: bb.phase >= 2` — o mesmo campo órfão que matava o Classic No.10, portanto um ramo que nunca corria.
- **Fluxograma da aba separada refeito (teamBtView.html):** layout novo (barra de topo com equipa/estado/controlos, diagrama, e cinco cartões em rodapé: Legenda, Tipos de Nós, Como Ler o Fluxo, Controlos, Informações). Os botões fazem coisas: Zoom (± e Ctrl+roda, até 2.6×), **Ajustar** — que passou a PREENCHER a janela em vez de só encolher (era `Math.min(1, …)`, e numa janela larga o diagrama ficava pequeno ao centro) —, Buscar por nome, e opções para esconder condições, esconder ramos não percorridos e congelar. O palco encosta à esquerda (`transform-origin: top left`), porque centrado o zoom empurrava metade da árvore para fora. Mostra o **Estado táctico** em vez da postura. Aviso de "sem sinal" ao fim de 3 s sem mensagens do jogo — antes ficava a árvore apagada sem dizer porquê. Testes: `tests/teambt_view.test.js`, incluindo um que compara os nomes dos nós com os de `team_bt.js` (a página espelha a árvore à mão) e outro que impede as posturas de voltarem.
- **Velocidade de giro do corpo (`SteeringModel.giro`, js/config.js):** o slerp estava fixo em `5.5 * delta` no `steerArrive`. Passou a constante e subiu para **10.0** — pedido explícito. O valor tinha andado de 7.0 para 3.5 e depois 5.5 à procura de um "feeling" pesado, e o efeito era o jogador demorar a apontar para onde ia, ou seja deslocar-se de lado enquanto rodava.
- **Tecto do corpo passou a depender da VELOCIDADE (`SteeringModel`, js/config.js + `steerArrive`, js/player.js):** o tecto fixo de 65° não chegou. Medido por estado da FSM (`tools/diag_bugs.js`): `SUPPORT_PASS` com **44.7°** de média e 32% das amostras acima de 60°, `BLOCKING` com **57.3°** e 61% — dentro do tecto, e a correr de lado na mesma. Agora o tecto interpola entre `desvioMaxAndar` (90°, até `velAndar` 2.5 m/s: parado ou a andar pode virar-se todo para a bola) e `desvioMaxCorrida` (22°, a partir de `velCorrida` 5.5 m/s). Resultado: geral acima de 60° **4.7% → 1.4%**; SUPPORT_PASS **23.3°/4.8%**; BLOCKING **30.9°/15.6%**. O `PASS` continua alto (60°) de propósito — é o `case 'PASS'` a rodar o jogador para o alvo do passe.
- **Elevação do chutão do guarda-redes (`puntBall`, js/player.js):** era `(25 + rand*25) / 3`, ou seja **8.3 a 16.7 graus** — uma bola esticada, quase a meia altura. Passou a **25 a 35 graus**, pedido explícito. Como a velocidade sai resolvida a partir do alcance (`R = v² sin2θ / g`), subir o ângulo **não** alonga o chutão: torna-o mais alto e mais lento, que é o que um tiro de meta deve ser.
- **Player_Pass_Points: os colegas sem pontuação são descartes, e agora dizem-no (`findPassTarget`, js/player.js):** os `continue` do ciclo de candidatos saltavam a linha que escreve os pontos, por isso quem fosse descartado ficava sem `Pass:` nenhum — parecia falha do botão. Passam a escrever o motivo: **`longe`** (acima de `max(10, skill*0.6)` metros), **`perto`** (abaixo de 2 m do ponto ou 3 m do colega) e **`linha`** (alvo em cima da linha lateral/de fundo). Continuam sem nada os que nem chegam ao ciclo: quem está a mais de 35 m atrás do portador, e toda a equipa sem posse.

- **A interceptação estava ao contrário (`pickIntercetor`, js/bt/team_bt.js):** dois defeitos no mesmo sítio, medidos com `tools/diag_bugs.js` (novo). (1) A função só desistia se houvesse `Match.ballCarrier` — e durante um passe não há portador nenhum, por isso a equipa que **acabou de passar** também escolhia intercetor: via-se o destinatário a ir ao ponto dele e um colega a ir ao ponto de intercepção, dois jogadores da mesma equipa a correr para sítios diferentes atrás da mesma bola. (2) O `tQuemJaVai` — o tempo de "quem já vai à bola", que um candidato tem de bater para valer a pena interceptar — incluía o `Match.intendedReceiver`, **mesmo sendo ele do adversário**. Como o destinatário chega quase sempre primeiro ao passe que lhe é dirigido, a equipa que defendia desistia quase sempre. Medido antes: alguém em INTERCEPT em **23%** dos frames na equipa COM posse contra **6%** na que defendia. Depois: **15.8% / 12.6%** — o que sobra do lado da posse são bolas soltas, onde ir à bola é legítimo. Testes: `tests/bugs_relatados.test.js`.
- **O botão Player_Pass_Points não mostrava nada (`limparPontosDeDebug`, js/match.js):** o `debugPoints` era limpo nas duas equipas no **topo** de `Match.update`, e quem os escreve é o portador, dentro do BT dele, a meio do frame — todo o colega processado ANTES do portador desenhava o label já apagado, e no frame seguinte era apagado outra vez. A limpeza passou a ser só da equipa **sem** posse (a equipa com bola mantém o último valor conhecido até o portador o reescrever). Medido: jogadores com pontos escritos no fim do frame **0 → 40**.
- **Corpo a 90 graus da direcção da corrida (`steerArrive`, js/player.js + `SteeringModel`, js/config.js):** nos estados `MARKING`, `BLOCKING`, `AFT_SUPPORT` e `SUPPORT_PASS` o jogador vira-se para a BOLA — o que é certo a marcar e a colocar-se, e absurdo a 7.9 m/s. Medido: **2.0%** dos frames acima de 90 graus entre o corpo e a direcção do movimento, e as amostras eram todas destes estados. O desvio passou a ter tecto (`SteeringModel.desvioMaxCorpo`, 65°) acima de `velAndar` (2.5 m/s): a colocar-se vira-se todo, a correr abre o corpo até ao tecto. Ficou em **0.7%**, e o que resta é a velocidades baixas noutros estados — inércia de quem inverte a marcha, não corpo trocado.
- **Extra Frontman deixou de subir em todo o ataque (`PlayingStyleTriggers.extra_frontman`, js/playing_styles.js):** a condição era `atacando && ultimoTerco && diferença de golos < 2`, que com o jogo empatado é quase sempre verdadeira no último terço — medido, o central subia em **13.2%** de todo o tempo de jogo. Pedido explícito: só na **bola parada da equipa dele** (canto, falta — e aí sobe mesmo fora do último terço) e **a perder no fim do jogo** (`ExtraFrontmanModel.minutoFinal`, minuto 75). Em jogo corrido passou a 0%.
- **Centrais a trocar de marcação: medido, e é raro.** A queixa foi verificada com uma métrica própria (cada CB estar mais perto do homem que o OUTRO CB marca): **0.2-0.5%** dos frames em que os dois marcam homens diferentes. Fica registado como não accionável até aparecer um caso reproduzível — o leilão guloso de `atribuirMarcacoes` continua a garantir um marcador por adversário.
- **A correcção da interceptação custou passes, e a causa é legítima:** a taxa de passe completo passou de **69.2 ± 6.8%** para **65.0 ± 6.7%** (20 sementes × 240 s), com o lançamento a cair para 56% e os cortes a subirem para 40%. Não é uma regressão de qualidade: antes a equipa que defendia quase **não tentava** interceptar, e a posse estava inflacionada por esse defeito. Qualquer meta de percentagem de passe daqui para a frente deve ser lida contra este número, não contra os 71.6% medidos com a interceptação avariada.
- **Testes em jogo instáveis (a saber, para não se perder tempo):** `o centro do bloco fica sobre a bola nas duas equipas` e `os defesas não se embolam com a bola na ala direita` passam isolados e falham (nem sempre os mesmos) dentro da suíte completa. Correm o jogo sem semente fixa, por isso variam entre execuções. Os diagnósticos em `tools/` usam PRNG semeado exactamente para não terem este problema.

- **Movimentação dos Playing Styles, medida antes de ser afinada (`tools/diag_posicoes.js`, novo):** ferramenta que corre o jogo em headless com semente fixa e mede, por posição e estilo, onde o jogador REALMENTE está — mediana do lado (`xLado`, positivo = do seu lado da formação), do avanço (`z * dirZ`), fracção de tempo na ala, dentro da grande área, e do mesmo lado do parceiro de ataque. Tem duas tabelas: a de todos os frames e a **só dos frames com o estilo LIGADO**, que é a única que responde a "quando o estilo manda, para onde é que ele leva o jogador". As quatro afinações abaixo saíram desta medição, não de impressões.
- **`%estOn` — o número que limita todos os estilos:** com a equipa a atacar, os estilos de movimentação estão LIGADOS entre 5% e 42% do tempo (Dummy Runner 7-8%, Roaming Flank 11-22%, Offensive Full-back 25-37%, Classic No.10 42%). Duas travas em série: o gatilho de cada estilo (`PlayingStyleTriggers`) e a **regra de estados tácticos** em `avaliarEstilo` (playing_styles.js), que desliga tudo o que não seja `TeamState.OFFENSIVE` — e OFFENSIVE exige **3 s de posse ininterrupta**, que só acontece em **50-53%** dos frames com bola. Afinar números de um estilo rende no máximo a fracção de tempo em que ele está ligado; é o tecto de qualquer afinação futura de movimentação.
- **Lateral ofensivo sobe e fica na ala (`offensive_fullback`, js/config.js + `colaNaLinha`, js/playing_styles.js):** medido, a mediana do avanço com a equipa a atacar era **-9 m** — o lateral "ofensivo" não passava do meio-campo. Três causas somadas: (1) só tinha `avanco: 4` e nenhum `avancoComBola`; (2) o gatilho exigia `bolaAvanco > -5`, ou seja a bola já no meio-campo, e por isso ele nunca arrancava com a construção a sair de trás — passou a `-18`; (3) o tecto da ala do `colaNaLinha` era **a borda do bloco**, e com o bloco basculado para a ala contrária essa borda fica perto do eixo — o "cola na linha" colava-o ao MEIO. Novo campo `larguraMin` (piso de largura absoluto, só travado pela linha lateral) resolve o (3) sem repor o defeito que a borda do bloco tinha vindo corrigir. Agora, com o estilo ligado: **88-91% do tempo na ala**, mediana do lado 23.6-25.3 m.
- **Roaming Flank deixa de se infiltrar por sistema (`roaming_flank`, js/config.js):** tinha `largura: -4`, que fechava 4 m para o eixo **sempre — com bola e sem ela**. Era isso que o punha no meio a marcar. Passou a `0`: a ponta é o posto dele e fechar é a excepção. A infiltração vive toda em `fechaComBolaCentral`, que só corre com a equipa a atacar e agora por **dois** motivos — o corredor tapado à frente (já existia) ou uma **linha de passe interior aberta** desde o portador (`linhaLivre`, novo, pedido explícito).
- **Box-to-Box: o tecto da entrada da área passou a valer para os outros destinos (`limitarPontoAoEstilo`, js/playing_styles.js):** o `aplicarTectoDoEstilo` só via o alvo posicional. Medido, das amostras dentro da grande área 19% eram `RUN_INTO_SPACE` e 16% (Dummy Runner) `SUPPORT_PASS` — dois destinos escritos noutros sítios (`p.apoioPonto` em team_bt.js, destino da corrida em player_bt.js) e que por isso furavam o tecto que o estilo declara. A mesma função corta agora os três.
- **Dummy Runner abre espaço em vez de o roubar (`dummy_runner` + `ladoOpostoAoParceiro`, js/playing_styles.js):** medido, estava **62-80% do tempo do mesmo lado do outro avançado** e com a mediana de avanço ACIMA da do Fox in the Box (10.5 vs 8.4) — o chamariz à frente do finalizador e no mesmo corredor. `largura` 5 → 3, `atraiDefesa` empurra 6 → 3 m, `avanco: -2`, e a bandeira nova espelha o alvo para o lado contrário ao do parceiro de ataque (`parceiroDeAtaque`, o colega `role: 'atk'` mais avançado). Com o estilo ligado passou a ficar **bem atrás** do Fox (avanço -4.0 vs 33.9 no TeamA, 19.4 vs 31.4 no TeamB).
- **`classic_no10` era código morto (`PlayingStyleTriggers`, js/playing_styles.js):** o gatilho exigia `bb.phase >= 2`, e o `phase` é escrito **uma vez** (`= 1`, no construtor do `TeamBlackboard`) e nunca mais — sempre falso, estilo nunca ligava, medido a **0%** do tempo. Mesmo padrão dos campos órfãos deixados pelo `position_bt.js` apagado. "Posse instalada" passou a ser lida onde ela existe mesmo (a regra de estados tácticos já exige `OFFENSIVE`, que são os 3 s de posse), e o gatilho ficou `atacando && !isCounter` — agora liga 42% do tempo com bola.
- **`distanceToSq` não existe em three (js/bt/player_bt.js:670):** `p.model.position.distanceToSq(...)` no ramo do giro de 180° com a bola — o método é `distanceToSquared`. Nunca tinha rebentado porque o ramo nunca era alcançado; passou a sê-lo com os estilos novos e o jogo abortava com `TypeError`. Corrigido.
- **Estilos por equipa (`EstiloPorEquipa`, js/config.js + `aplicarPlayingStyle`, js/match.js):** o `EstiloPorOmissao` é indexado só pela posição, por isso os dois onzes saíam clones um do outro. A tabela nova é consultada ANTES do omisso e é indexada por equipa; o que lá não estiver cai no omisso da posição, e um estilo pedido que não sirva à posição também cai lá (não deixa o jogador sem estilo). Atribuição actual: **TeamA** RB Defensive Full-back, CB2 The Destroyer, CM2 Classic No.10, LM Creative Playmaker, RM Prolific Winger; **TeamB** RB Defensive Full-back, LM Cross Specialist, RM Roaming Flank. `prolific_winger` passou a aceitar LM/RM (num 4-4-2 não existe LW/RW). **Armadilha medida:** a primeira atribuição pôs os dois alas mais ofensivos na mesma equipa e o jogo ficou **63/37 na posse**, com dois testes em jogo a falhar por arrasto; repartir um ala ofensivo por equipa repôs 47/53 e os testes.
- **Efeito colateral a decidir:** a taxa de passe completo passou de **71.6 ± 3.8%** para **69.2 ± 6.8%** (20 sementes × 240 s, mesma medição). A maior parte vem dos cruzamentos, que subiram em número (45 → 62 amostras) e caíram de 67% para **44%** de acerto — o Cross Specialist e o Prolific Winger cruzam mais, e o cruzamento é o tipo de passe com pior taxa. A variância entre sementes quase dobrou, o que é esperado com os dois onzes a jogar de maneiras diferentes.
