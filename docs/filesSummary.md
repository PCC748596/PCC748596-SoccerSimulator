...
### Sessão de 1 de Setembro de 2026 — o tiro de meta e o lateral do lado contrário

**No tiro de meta a equipa que bate vai, no mínimo, para as posições do
TeamBT.** Estavam a cair em `SET_PIECE_WAIT` — a folha que congela o jogador
onde está e lhe escreve o alvo — e por isso ficavam onde a jogada os tinha
deixado: medido, **18,6 m de média até ao slot do bloco** (p95 36,3), com 79%
dos alvos vindos dessa folha, e os centrais a -44,9 m de avanço (oito metros da
própria linha de fundo). Nas imagens via-se um deles dentro da própria baliza.

Três coisas, e as três medidas:

- quem bate fica agora em **MOVE_TO_POS sem escrever alvo nenhum**: sem
  proposta da árvore, quem manda é a camada posicional (ESTRUTURA, ver
  `js/bt/alvo.js`) — ou seja, o slot no bloco;
- a montagem deixou de impor um desenho próprio por cima disso;
- as duas guardas do lance — fora da própria área, fora do corredor do chuto —
  passaram a ser aplicadas AO SLOT, na camada posicional. A da área é por
  PROFUNDIDADE e não pelo rectângulo: a traseira do bloco encosta à linha da
  área, o slot cai em cima dela e o corpo entrava.

```
distância ao slot do TeamBT   18,6 -> 10,9 m   (p95 36,3 -> 24,8)
alvos vindos do slot            21% ->   91%
dentro da própria área        22,8% ->  2,1%
```

(Os 10,9 m que sobram são caminho: o lance dura ~16 s e eles vão a andar para
lá.)

**E o lateral do lado contrário estava sozinho no outro lado do campo.**
Medido: **51% dos ALVOS dele fora do rectângulo do bloco em x** e 15,6 m até ao
colega mais perto (p95 27,0).

Eram duas regras a somar-se: ele está FORA do pêndulo (cobre o extremo
adversário, não vai atrás da bola) e a regra da faixa manda o "de fora" ficar
por fora do parceiro — do lado contrário à bola, o de fora é ele. Sem tecto,
isso empurrava-o para fora do bloco. Tirar-lhe a regra resolvia isso e criava
outro problema: **31% das leituras com ele no eixo do campo** (|x| < 5 m).

Ficou com as duas coisas: por fora do extremo, mas nunca mais de
`BlockShape.folgaLateralOposto` (1,5 m) para lá da borda do bloco, e com tecto
de pêndulo próprio (`distanciaMaxXLateralOposto`, 30 m contra os 22 dos
outros).

```
alvo fora do bloco em x        51%  ->   9%
colega mais perto            15,6 m -> 13,0 m   (p95 27,0 -> 20,6)
no eixo do campo (|x| < 5)     31%  -> 3,9%
```

Fica dito o que a troca custou: o lateral por dentro dos centrais subiu de 1,6%
para 8,2% e o "lado errado" de 0,4% para 4,4% — é o preço de o manter ligado ao
bloco em vez de o deixar colado à linha. O botão é o `folgaLateralOposto`.

**E a bola assenta 10 cm mais baixa no cone** (`MultiBallModel.encaixeNoCone`):
encaixada nele, não pousada em cima.

### Sessão de 1 de Setembro de 2026 — a bola que sai, e a saída a jogar

Quatro afinações do lance parado, todas de ver o jogo.

**A bola que sai continua o movimento.** Parava em cima da linha porque o prop
da bola morta era estático. Agora sai com a velocidade que trazia e tem física
própria — gravidade, arrasto, ressalto (`restituicao` 0,45) e atrito de
rolamento — e os **3 s contam a partir do momento em que PÁRA**, não de quando
saiu: uma bola que sai a 20 m/s ainda tem campo fora para percorrer.

**As bolas de reserva são iguais às do jogo.** Passaram a ser criadas pelo
`Match.criarBola` — a mesma malha do `assets/Ball.obj` — em vez de uma esfera
lisa. E os cones ficaram mais pequenos: raio 0,22 → 0,13, altura 0,34 → 0,20,
que é a proporção de um cone de treino ao lado de uma bola de 0,11.

**Com o jogo parado ninguém corre atrás da bola.** O `pickIntercetor` já tinha
a guarda `Match.state !== 'PLAY'`; o `pickChaser` não — e com a multibola isso
piorou, porque a bola do jogo passou a acompanhar quem a foi buscar (fica
invisível, mas a POSIÇÃO tem de acompanhar: meia dúzia de sítios leem
`Match.ball.position`). Sem a guarda, meio campo ia atrás dela até ao cone.
Medido depois: **0,00 equipas com chaser** e **0,00 jogadores fora do campo**
por frame com o jogo parado.

**E a equipa oferece-se quando o guarda-redes segura a bola.** O
`atribuirApoiosDaEquipa` saía logo à entrada nesse caso: medido, **zero
jogadores a oferecer-se em 100% dos frames** com ele a segurar, e a equipa a
encolher para 36,3 m de largura — ele repunha para ninguém. O motivo do corte
era bom (não se quer o médio dentro da área, colado a ele) mas a resposta era
grande de mais; agora é um FILTRO: os apoios existem, nenhum dentro da própria
área e nenhum a menos de `SupportModel.distMinAoGuardaRedes` (14 m).

    apoios por frame com o GR a segurar    0,00 -> 1,32
    frames com pelo menos um                 0% -> 91%
    largura da equipa                     36,3 -> 38,4 m

Testes: `tests/multibola.test.js` passa a 13 casos.

### Sessão de 1 de Setembro de 2026 — lançamento não é passe no espaço

Relato: *"o THROUGH não está fazendo sentido, está saindo um passe de 10 metros.
O through é um lançamento longo que passa entre adversários; diferente do pass
into space, que é um passe na frente que não passa entre adversários."*

Estava tudo misturado num sítio só: o `aplicarMiraDoPasse` (player_bt.js)
marcava `isThroughBall` para QUALQUER passe do tipo SPACE/LEADING. Daí a
etiqueta THROUGH em bolas de 10 m — e daí também as estatísticas de lançamento
inflacionadas.

Passaram a ser dois conceitos, com o teste geométrico que faltava:

    LANÇAMENTO       bola longa (>= PassModel.distMinLonga, 30 m) que sai POR
                     ENTRE dois adversários e cai nas costas da linha
    PASSE NO ESPAÇO  bola à frente, para espaço LIVRE, que não passa entre
                     ninguém

O teste é o `passaEntreAdversarios` (utils.js): tem de haver um adversário de
CADA LADO da linha do passe, os dois dentro do segmento e a menos de
`throughBallCorredorLargura` (7 m) dela, com a soma das duas distâncias até
`throughBallVaoMax` (13 m). Mais do que isso não é um corredor entre dois
homens, é campo aberto.
...
Antes, os 120 apareciam como THROUGH.

A balística continua a ser a mesma nos dois (a bola vai para um PONTO, não para
os pés): o `executePassGameplay` aceita as duas marcas. O que muda é o nome, a
etiqueta no ecrã (`THROUGH` contra `SPACE`), o painel do BT (`Lancar` contra
`Espaco`) e as estatísticas — só o lançamento conta como lançamento.

**E o golo passou a ter o estádio.** `assets/Soccer_Crowd_Cheerin.mp3`, em
`EfeitosSonoros.golo()`, disparado com o apito no momento do golo. Volume 0.75
(acima do apito, é a única altura em que a bancada tapa tudo), sem atenuação
por distância — a bancada não está onde a bola está — e com 3 s de intervalo
mínimo, para dois golos seguidos em 10x não sobreporem duas multidões.

Teste: `tests/lancamento_vs_espaco.test.js` (10 casos), que corre o
`passaEntreAdversarios` a sério (um lado, dois lados, vão largo, adversários
fora do segmento, linha em qualquer direcção) e fixa o encaminhamento das duas
marcas e o som do golo.

### Sessão de 1 de Setembro de 2026 — multibola, tiro de meta e minimapa

**MULTIBOLA (`js/multiball.js`, novo).** Dezoito bolas em cones à volta do
campo — fila em cada linha lateral (a do lado dos bancos salta o meio-campo) e
quatro atrás de cada baliza, sem nenhuma em frente aos ferros. Quando a bola
sai, fica onde caiu e é retirada **3 s** depois; quem vai bater o lateral, o
canto ou o tiro de meta vai buscar a reserva mais perto da marca, leva-a na mão
e pousa-a lá. Só então o lance começa a contar tempo. **O jogo não pára**: as
duas equipas reorganizam-se enquanto isso acontece, como já faziam.

    MultiBall.maisProxima(x, z) / .retirar() / .repor()
    MultiBall.largarBolaMorta(pos)      a bola que saiu, 3 s no relvado
    Match.iniciarReposicao(tipo, batedor, marca, aoPousar)
    Match.atualizarReposicao(dt)        corre no match_loop, em qualquer lance

Sem reservas livres (ou sem o módulo carregado) cada lance cai no comportamento
antigo — bola na marca, sem espera. O `SetPieceReposicao.prazo` (12 s) é a rede
de segurança se o batedor não lá chegar.

Medido, com o lance completo do início ao fim:

    lateral        8/8 com reserva, 8/8 completos, 8,9 s de média
    canto          8/8                8/8           12,0 s
    tiro de meta   8/8                8/8           16,3 s

E em 10 minutos de jogo corrido: 9,1% dos frames em reposição, a mais longa em
11,9 s, **zero** frames com a bola invisível fora de uma reposição, as 18
reservas de volta aos cones no fim, 111/155 passes e 3 golos (o normal).

**O batedor do lateral já não aparece na linha.** Era teleportado para lá no
mesmo frame; agora vai buscar a bola e só assume a posição regulamentar (fora
do campo, atrás da linha) quando lá chega com ela. Pedido explícito.

**Tiro de meta: a área é do guarda-redes.** Medido: **1,87 companheiros dentro
da própria grande área** por frame durante o lance (pior caso 4), um deles a
4,6 m dele e um central em cima da linha de fundo — é o que se vê na imagem. O
alvo deles vinha da formação avançada 6 m, e para um central isso cai dentro da
área. As duas guardas — fora da área (`GoalkeeperPose.tiroMetaMargemForaDaArea`)
e fora do corredor do chuto (`.tiroMetaCorredorDoChuto`) — nasceram aqui, na
montagem, e **mudaram de sítio mais tarde nesta mesma sessão**: passaram a ser
aplicadas ao SLOT do TeamBT, na camada posicional (ver a secção do tiro de meta
mais acima, que é a versão que está no código).

    dentro da própria área   1,87 -> 0,02 por frame
    mais perto do GR          4,6 m -> 9,2 m

**Minimapa 70% maior.** `Minimap.raioJogador` 2.6 -> 4.4 e `raioBola` 1.8 ->
3.1. Só o desenho dos pontos; a escala do campo é a mesma.

Testes: `tests/multibola.test.js` (9 casos) — corre o módulo com uma cena falsa
(posições fora das linhas, escolha da mais próxima, regresso ao cone, os 3 s da
bola morta) e verifica o encaminhamento dos três lances, as saídas de recurso,
o fim do teleporte no lateral, as guardas do tiro de meta e o tamanho dos
pontos do minimapa.
...
three.min.js (CDN)
  ...
  └─ event_bus.js → joint_limits.js
       ...
       → multiball.js → minimap.js → officials.js → crowd.js
       → bt/btDebug.js
       → main.js
```
...
| O que é lançamento e o que é passe no espaço | `utils.js` → `passaEntreAdversarios`; `config.js` → `PassModel.throughBallVaoMax` / `.throughBallCorredorLargura` |
| Som da bancada no golo | `efeitos_sonoros.js` → `golo()`; `assets/Soccer_Crowd_Cheerin.mp3` |
| Onde a equipa que bate se põe no tiro de meta | o slot do TeamBT, com as guardas em `config.js` → `GoalkeeperPose.tiroMetaMargemForaDaArea` / `.tiroMetaCorredorDoChuto` |
| Lateral do lado contrário desligado da equipa | `config.js` → `BlockShape.folgaLateralOposto` / `.distanciaMaxXLateralOposto` |
| Equipa parada quando o guarda-redes segura a bola | `config.js` → `SupportModel.saidaComGuardaRedes` / `.distMinAoGuardaRedes` |
| Bola a parar em cima da linha ao sair | `js/multiball.js` → `MultiBallModel.restituicao` / `.atritoSolo` |
| Altura da bola em cima do cone | `js/multiball.js` → `MultiBallModel.encaixeNoCone` |
| Bolas de reserva nos cones (multibola) | `js/multiball.js` → `MultiBallModel`; `Match.iniciarReposicao` |
| Quanto tempo a bola que saiu fica no relvado | `js/multiball.js` → `MultiBallModel.prazoBolaMorta` (3 s) |
| Batedor a demorar a repor a bola | `config.js` → `SetPieceReposicao` (prazo, distâncias) |
| Companheiros dentro da área no tiro de meta | `config.js` → `GoalkeeperPose.tiroMetaMargemForaDaArea` / `.tiroMetaCorredorDoChuto` |
| Tamanho dos pontos do minimapa | `minimap.js` → `raioJogador` / `raioBola` |
