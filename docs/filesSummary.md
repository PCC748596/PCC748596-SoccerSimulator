...
- **O árbitro corre sempre de cara para a bola** (`mover`, js/officials.js): virava-se para onde andava, e como o alvo dele é uma projecção na diagonal do campo, passava metade do lance a correr de costas para a jogada. O `mover` ganhou um argumento `olharPara` opcional; os assistentes continuam a virar-se para o movimento. A orientação é escrita mesmo com ele parado, por isso não passa pelo corte de `d > 0.4` que a viragem para o movimento tem.
- **E ganhou o passo lateral dos jogadores** (`LateralGait` aplicado no `mover`): com o corpo preso à bola e o alvo noutro sítio, o ciclo de passada frontal ficava a correr por cima de uma deslocação de lado e o árbitro patinava. Mesma conta do `animateBones` — `lateralidade` de 0 a 1, passada encolhida por `reducaoPassada` e abdução das ancas em oposição de fase.
